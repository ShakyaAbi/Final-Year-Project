...
export const createIndicator = async (
  ...
  if (data.dataType === 'CATEGORICAL') {
    if (!data.categories || data.categories.length === 0) {
      throw new BadRequestError('MISSING_CATEGORIES', 'Categories are required for CATEGORICAL indicators');
    }
    validatedCategories = validateCategories(data.categories);
    validatedCategoryConfig = data.categoryConfig ? validateCategoryConfig(data.categoryConfig) : {};
  }
...
const calculateTrend = (values: number[]): 'increasing' | 'decreasing' | 'stable' | null => {
...
export const detectReportingGaps = (
  ...
  for (let i = 1; i < sorted.length; i++) {
    const daysDiff = (sorted[i].reportedAt.getTime() - sorted[i-1].reportedAt.getTime()) / (1000 * 60 * 60 * 24);
...
export const bulkUpdateIndicators = async (
  projectId: number,
  updates: Array<{ id: number; data: Partial<{
    logframeNodeId: number;
    name: string;
    unit: string;
    baselineValue: number | null;
  ...
  if (indicators.some(ind => !ind || ind.projectId !== projectId)) {
    throw new BadRequestError('INVALID_INDICATOR', 'All indicators must belong to the specified project');
  }
  ...
  return prisma.$transaction(
    updates.map(({ id, data }) => 
      indicatorRepo.updateIndicator(id, data)
    )
  );
};
