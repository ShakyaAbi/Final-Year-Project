...
export const IndicatorCard: React.FC<IndicatorCardProps> = ({ indicator, onEdit }) => {
  // Get latest value
  const sortedValues = [...indicator.values].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  const latestValue = sortedValues.length > 0 ? sortedValues[sortedValues.length - 1].value : indicator.baseline;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow p-6 flex flex-col h-full">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`text-xs font-medium px-2 py-1 rounded-full ${getStatusColor(indicator.status)}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-slate-50 rounded-lg border border-slate-100">
        <div>
          ...
          <p className="text-2xl font-bold text-slate-900 mt-1">{formatValue(indicator.target)}</p>
        </div>
        <div>
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Current</span>
          <p className="text-2xl font-bold text-slate-900 mt-1">{formatValue(latestValue)}</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-slate-100 mt-auto">
        <Link to={`/indicators/${indicator.id}`} className="flex-1">
          <Button variant="secondary" size="sm" className="w-full bg-slate-100 hover:bg-slate-200 border-none text-slate-600">
            View Details
          </Button>
        </Link>
        <Button variant="outline" size="sm" className="px-4 text-slate-500 hover:text-slate-700">
          Edit
        </Button>
      </div>
    </div>
