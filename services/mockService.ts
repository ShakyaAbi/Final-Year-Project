...
import { Project, LogframeNode, Indicator, IndicatorType, ProjectStats, ActivityLog, UserProfile, NodeType, IndicatorValue } from '../types';
...
const MOCK_PROJECTS: Project[] = [
  {
    ...
    description: 'Improving crop yields and market access for smallholder farmers in the Northern District through training and infrastructure.',
    ...
    logframe: [
      {
        ...
        description: 'To increase the average household income of participating farmers by 30%.',
        indicatorCount: 2,
        children: [
          {
            id: 'oc-1',
            ...
            children: [
              {
                ...
                description: 'Number of farmers who completed the 3-day workshop.',
                indicatorCount: 1,
                children: [
                   {
                      id: 'a-1',
                      ...
                      description: 'Hold weekly training sessions in community centers.',
                      indicatorCount: 0,
                      children: []
                   }
                ]
  ...
  },
  {
    ...
    description: 'Drilling boreholes and establishing water committees in 50 rural communities.',
  ...
  },
  {
    ...
    description: 'A pilot program to teach coding and digital literacy to unemployed youth.',
...
const MOCK_INDICATORS: Indicator[] = [
  ...
  },
  {
    ...
    values: [
      ...
      { id: 'v-9', date: '2024-02-07', value: 65, isAnomaly: true, anomalyReason: 'Value exceeds max expected (50)' },
    ]
  }
];

let MOCK_ACTIVITIES: ActivityLog[] = [
  { id: 'l-1', user: 'Jane Doe', userInitials: 'JD', action: 'updated', item: 'Project Timeline', date: '2 hours ago', type: 'info' },
  { id: 'l-2', user: 'System', userInitials: 'SYS', action: 'detected', item: 'Anomaly in Income Data', date: '5 hours ago', type: 'warning' },
  { id: 'l-3', user: 'Mike Smith', userInitials: 'MS', action: 'completed', item: 'Q1 Report', date: '1 day ago', type: 'success' },
];
...
const findAndAddNode = (nodes: LogframeNode[], parentId: string, newNode: LogframeNode): boolean => {
...
const findAndUpdateNode = (nodes: LogframeNode[], nodeId: string, data: Partial<LogframeNode>): boolean => {
    for (const node of nodes) {
        if (node.id === nodeId) {
...
export const addLogframeNode = async (projectId: string, parentId: string | null, nodeData: LogframeNode) => {
    await delay(200);
...
export const updateLogframeNode = async (projectId: string, nodeId: string, nodeData: Partial<LogframeNode>) => {
    await delay(200);
    const project = _projects.find(p => p.id === projectId);
    if (!project) throw new Error('Project not found');
...
export const getIndicators = async (projectId?: string): Promise<Indicator[]> => {
    await delay(300);
    if (projectId) {
...
export const createIndicator = async (indicator: Partial<Indicator>): Promise<Indicator> => {
    await delay(400);
...
export const saveValue = async (indicatorId: string, value: number | string, date: string, evidence?: string) => {
    await delay(400);
    ...
    if (ind) {
        ...
        if (!isNaN(numVal) && (ind.minExpected !== undefined && ind.maxExpected !== undefined)) {
